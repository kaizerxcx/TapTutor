...
public class TriangleScript : MonoBehaviour
{
    ...
    [SerializeField] private Transform trianglePlace =null;
        ...
        if (Input.touchCount > 0)
        {
            ...
            switch (touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if (Mathf.Abs(transform.position.x - trianglePlace.position.x) <= 0.5f &&
                        Mathf.Abs(transform.position.y - trianglePlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(trianglePlace.position.x, trianglePlace.position.y);
